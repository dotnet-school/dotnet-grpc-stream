...
function greetManyTimes(call, callback) {
  ...
  setInterval(function() {
      ...
      greetManyTimesResponse.setResult(`Initiated-by:${firstName},count=${i++}`);
...
function main() {
  let credentials = grpc.ServerCredentials.createSsl(
    fs.readFileSync(process.cwd() + "/certs/ca.crt"),
    [
      {
        cert_chain: fs.readFileSync(process.cwd() + "/certs/server.crt"),
        private_key: fs.readFileSync(process.cwd() + "/certs/server.key")
      }
    ],
    true
  );
